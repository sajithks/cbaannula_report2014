...
print(current_fig,['-r',num2str(74.99047619*K)], '-dpng', 'temp.png');
...
if strcmp(aamethod,'standard')
    ...
    kk = kk./sum(kk(:));
    a1 = max(min(myconv(single(raw_hires(:,:,1))/(256),kk,'same'),1),0);
    a2 = max(min(myconv(single(raw_hires(:,:,2))/(256),kk,'same'),1),0);
    a3 = max(min(myconv(single(raw_hires(:,:,3))/(256),kk,'same'),1),0);
